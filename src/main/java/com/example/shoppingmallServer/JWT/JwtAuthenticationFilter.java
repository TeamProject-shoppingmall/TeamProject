...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
        ...
        if(accessToken != null) {
            ...
            // 어세스 토큰이 만료된 상황 && 리프레시 토큰 또한 존재하는 상황
                ...
                if (isRefreshToken) {
                    ...
                    String loginId = jwtTokenProvider.getIdFromToken(refreshToken);
                    // 새로운 어세스 토큰 발급
                    String newAccessToken = jwtTokenProvider.createToken(loginId, "Access");
                    // 헤더에 어세스 토큰 추가
                    jwtTokenProvider.setHeaderAccessToken(response, newAccessToken);
                    // Security context에 인증 정보 넣기
                    setAuthentication(jwtTokenProvider.getIdFromToken(newAccessToken));
                }
    ...
    public void setAuthentication(String id) {
        Authentication authentication = jwtTokenProvider.createAuthentication(id);
    ...
    public void jwtExceptionHandler(HttpServletResponse response, String msg, HttpStatus status) {
        ...
        try {
            String json = new ObjectMapper().writeValueAsString(new ResponseEntity<>(msg, HttpStatusCode.valueOf(status.value())));
